...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # the body id for each foot for placement detection (Part 4.2)
        self._feet_ids = []
        for name in ["FL_foot", "FR_foot", "RL_foot", "RR_foot"]:
            ids, _ = self.robot.find_bodies(name)
            self._feet_ids.append(ids[0])
        # self._undesired_contact_body_ids, _ = self._contact_sensor.find_bodies(".*thigh")
        # Raibert & gait clock inputs (Part 4.2)
        ...
        obs = torch.cat(
            [
                ...
                for tensor in (
                    ...
                    self._actions,
                    self.clock_inputs, # Vibestep clock (part 4)
                )
    ...
    def foot_positions_w(self) -> torch.Tensor:
        ...
        Shape: (num_envs, num_feet, 3) (Part 4.3)
        """
        return self.robot.data.body_pos_w[:, self._feet_ids]

    def _step_contact_targets(self):
        """Update the gait phases: (Part 4.4)
        self.gait_indices、clock_inputs、desired_contact_states.
        """
        frequencies = 3.
        phases = 0.5
        offsets = 0.
    ...
    def _reward_raibert_heuristic(self):
        """Raibert heuristic for foot placement (Part 4.5)"""
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # update gait_indices、clock_inputs (part 4.6)
        ...
        rewards = {
            ...
            "raibert_heuristic": rew_raibert_heuristic * self.cfg.raibert_heuristic_reward_scale, # add Raibert heuristic reward (part 4.6)
        }
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Base height too low detection (Part 3.2)
        base_height = self.robot.data.root_pos_w[:, 2]
        cstr_base_height_min = base_height < self.cfg.base_height_min

        died = cstr_termination_contacts | cstr_upsidedown | cstr_base_height_min # added base height condition
